...
def main():
    ...
    if not PRACTICE_FILE.exists():
        ...
        print("Practice data is required before qualifying.")
        return

    if not QUALIFYING_FILE.exists():
        print("No live qualifying results detected.")
        print("Run download_live_sessions.py after qualifying.")
        return
    ...
    print(f"Qualifying model features: {len(model_features)}")
    print()
    ...
    for session_number in [1, 2, 3]:
        ...
        result[f"fp{session_number}_position"] = np.nan

    practice_sessions = {}

    for session_number in [1, 2, 3]:

        practice_sessions[session_number] = practice[
    ...
    for session_number, session_df in practice_sessions.items():
        ...
        if driver_column is None:
            raise ValueError(
                f"Could not identify drivers in FP{session_number}."
            )
    ...
    print(f"FP1 drivers matched: {matched[1]}/22")
    print(f"FP2 drivers matched: {matched[2]}/22")
    print(f"FP3 drivers matched: {matched[3]}/22")
    ...
    if qualifying_driver_column is None:
        raise ValueError(
            "Could not identify drivers in qualifying results."
        )
    ...
    if qualifying_position_column is None:
        raise ValueError(
            "Could not identify qualifying positions."
        )
    ...
    for index, row in result.iterrows():
        ...
        matches = qualifying[
            qualifying[qualifying_driver_column]
            .astype(str)
            .str.strip()
            .str.lower()
            ==
        ...
        if q1_column:
            result.at[index, "q1"] = numeric_value(
                q_row[q1_column]
            )

        if q2_column:
            result.at[index, "q2"] = numeric_value(
                q_row[q2_column]
            )

        if q3_column:
            result.at[index, "q3"] = numeric_value(
                q_row[q3_column]
            )
    ...
    # ---------------------------------------------------------------
    # Qualifying weather features
    #
    # These remain empty until actual live session weather is connected.
    # ---------------------------------------------------------------
    ...
    # Sprint features
    #
    # Italian GP is a normal weekend, so these must remain unavailable.
    # ---------------------------------------------------------------
    ...
    if missing:
        ...
        raise ValueError(
            "Missing required qualifying model features:\n"
            + "\n".join(
                f"  - {feature}"
    ...
    if matched_qualifying == 0:
        ...
        print("No valid qualifying positions detected.")
        return
    ...
    print()
    print("QUALIFYING FEATURE VALIDATION")
    print("-" * 80)
    ...
    print(f"Qualifying positions: {matched_qualifying}/22")
    print(
        f"Model features available: "
        f"{len(model_features)}/{len(model_features)}"
    )
    print("Race-finish leakage: PASS")
    print("Sprint-finish leakage: PASS")
    print()
